...
const QuestionnaireEditor = () => {
  ...
  const addFollowUpQuestion = (qIdx: number, aIdx: number) => {
    setQuestionnaire(prev => {
      ...
      // Insert just after the last follow-up related to this parentQ
  ...
  const saveChanges = () => {
    ...
    // 1. Create a deep copy of top-level questions
    ...
    // 2. Attach follow-ups to their respective parent answer
    ...
    // 3. Save the reconstructed structure
    ...
    fetch(`/api/questionnaires/${folder}/${filename}`, {
      ...
      .then(() => alert('Tallennettu!'))
      .catch(() => alert('Virhe tallennettaessa.'));
  };  
        ...
        {questionnaire.questions.map((q, qIdx) => {
          ...
          return (
            <React.Fragment key={q.id}>
              <div
                className={
                  isFollowUp ? `${styles.questionBlock} ${styles.followUpBlock}` : styles.questionBlock
                }
              >
                {isFollowUp && (
                ...
                {q.answers.map((a, aIdx) => (
                  <div key={aIdx} className={styles.answerBlock}>
                    ...
                    {!isFollowUp && (
                      <button className={styles.addBtnSmall} onClick={() => addFollowUpQuestion(qIdx, aIdx)}>+ Lisää jatkokysymys</button>
                    )}
                  </div>
                ))}
              </div>
